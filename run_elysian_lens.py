...
def setup_parser():
    ...
    parser.add_argument(
        "--url", 
        help="URL to scrape (for scrape mode)"
    )
    parser.add_argument(
        "--file", 
    ...
    parser.add_argument(
        "--prompt", 
        help="Prompt for text generation or agent (for generate/agent mode)"
    )
    parser.add_argument(
        "--model", 
        default="llama3",
...
def run_generate_mode(prompt, model, output=None):
    ...
    cmd = [sys.executable, "-m", "elysian_lens", "generate-text", prompt, "--model", model]
